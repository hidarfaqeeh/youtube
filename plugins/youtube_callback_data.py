    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "Audio", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("م",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "فديو", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("ملف",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("جاري التحميل من يوتيوب...", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("جاري رفع الفديو  ...", callback_data="down")]]))
